...
class djfaclHelperToolbar extends JObject

{
	...
	function customdjfacl($task = '', $alt = '', $cssClass = '', $link='') {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$title = JText::_ ( $alt );
		if($link==""){
	...
	function purge() {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$bar->appendButton ( 'Link', 'purge', 'Purge', 'index.php?option=com_djfacl&controller=gruppi&task=purge' );
	}
	...
	function excel() {
		...
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'excel', 'Excel', 'index.php?option=com_djfacl&controller=gruppi&format=xls' );
	}
	
	function import($nomeclass) {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'import', 'Import', 'index.php?option=com_djfacl&controller='.$nomeclass.'&task=import' );
	}
	function export($nomeclass) {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'export', 'Export', 'index.php?option=com_djfacl&controller='.$nomeclass.'&format=xls' );
	}
	
		function importJUser() {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$bar->appendButton ( 'Link', 'juser', 'Import JUser', 'index.php?option=com_djfacl&controller=gruppi_utenti&task=importJUser' );
	}
